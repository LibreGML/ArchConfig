...
10. 格式化EFI分区:`mkfs.fat -F32 /dev/nvme0n1p1`，格式化swap分区`mkswap /dev/nvme0n1p2`，`mkfs.btrfs -L myArch /dev/nvme0n1p3`。
11. 创建btrfs子卷:，先挂载btrfs分区到/mnt:， `mount -t btrfs -o compress=zstd /dev/nvme0n1p3 /mnt`，`df -h`查看是否挂载成功，`btrfs subvolume create /mnt/@ ` 创建根目录子卷，`btrfs subvolume create /mnt/@home` 创建home子卷，`btrfs subvolume list -p /mnt`，查看挂载情况。将/mnt卸载以挂载子卷，`umount /mnt`。
...
13. 安装基础包和基础软件: `pacstrap /mnt base base-devel linux linux-firmware btrfs-progs networkmanager neovim sudo zsh zsh-completions pacman`。内核构建可能报`file not found: /etc/vconsole.conf`，需要手动创建，`echo "KEYMAP=us" > /etc/vconsole.conf &&  echo "FONT=lat9w-16" >> /etc/vconsole.conf`
